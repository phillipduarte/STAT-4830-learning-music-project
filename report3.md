...
$h$ is a hidden representation with $H = 512$, $z$ are the logits. The model has approximately 395,000 trainable parameters. Kaiming uniform initialization is used for both linaer layers which accounts for the ReLU non-linearity — it scales weights by $\sqrt{2/\text{fan\_in}}$ so that the variance of activations is preserved through each layer at initialization. Dropout is placed after the ReLU activation, meaning that the activation decides which neurons fire and this step  stochastically zeros some of those active units, acting as regularization.
